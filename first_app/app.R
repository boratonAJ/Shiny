...
ui <- fluidPage(
   ...
   # Application title
   titlePanel("South Africa Data Stats"),
   ...
   sidebarLayout(
      sidebarPanel(
         selectInput(inputId = "province",
                     ...
                     selected =  "Gauteng", #unique(mortality_zaf$province)[[5]], #e.g Gauteng
                     multiple = TRUE
                     ),
        checkboxInput(inputId = "showtable", label = "Show tables?", value = FALSE)
      ),
...
server <- function(input, output) {
   ...
   output$LinePlot <- renderPlot({
     mortality %>% filter(province %in% input$province) %>% 
       ...
       facet_wrap(~province) +
       geom_line(alpha = 0.8, size = 1.5) + theme_minimal(base_size = 18)
     
   })
   output$mortalityTable <- renderDataTable({
     if(input$showtable){
          DT::datatable(data = mortality %>% 
